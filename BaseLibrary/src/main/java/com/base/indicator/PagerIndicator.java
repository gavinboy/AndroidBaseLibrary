...
public class PagerIndicator extends HorizontalScrollView {
	...
	public static final int SCROLL_MODE_LEFT=0;//当前tab保持在左边,只到右边滚到头
	public static final int SCROLL_MODE_RIGHT=1;//当前tab一直右移,直到到达右边屏幕
	...
	private int tabBackgroundResId = R.drawable.inticator_background_tab;

	private Locale locale;

	private boolean shouldSameWidth;//文字和Indicator宽度相同

	private int mScreenWidth;//屏幕宽度

	private int scrollMode=SCROLL_MODE_LEFT;//滚动模式
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			if(scrollMode==SCROLL_MODE_RIGHT){
				View mView=tabsContainer.getChildAt(position);
				int scrollLength=0;
				Log.d(TAG,"onPageSelected->position:"+position);
				if(position==tabCount-1){
					Log.d(TAG,"onPageSelected->smootScrllTo Right!");
					smoothScrollTo(mView.getLeft(),0);
				}else if(position==0){
					Log.d(TAG,"onPageSelected->smootScrllTo left!");
					smoothScrollTo(0,0);
				} else{
					if((mView.getRight()-getScrollX())+tabPadding>mScreenWidth){
						Log.d(TAG,"onPageSelected->smootScrllBy Right:"+mView.getRight()+" width:"+mScreenWidth);
						scrollLength=tabsContainer.getChildAt(position+1).getMeasuredWidth();
						smoothScrollBy(scrollLength,0);
					}else if(mView.getLeft()<getScrollX()){
						Log.d(TAG,"onPageSelected->smootScrllBy Left");
						scrollLength=(tabsContainer.getChildAt(position).getMeasuredWidth())*(-1);
						smoothScrollBy(scrollLength,0);
					}
